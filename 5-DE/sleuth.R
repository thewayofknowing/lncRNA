...
run_wt <- function(so, coeff, gene_mode = FALSE) {
  ...
  if (gene_mode) {
    filename <- file = paste(experiment_name, '_', coeff, '.gene.table.tsv', sep='')
  }
  else {
    filename <- file = paste(experiment_name, '_', coeff, '.table.tsv', sep='')
  }
...
run_sleuth <- function(experiment_name, gene_mode = FALSE) {
  experiment_filename <- paste(experiment_name, '.txt', sep='')
...
t2g <- read.table('../3-Transcriptome/pretty_merged_gtf.tsv', header = TRUE)
